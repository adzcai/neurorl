...
class LanguageEncoder(hk.Module):
    ...
    def __init__(
        ...
        self.embedder = hk.Embed(vocab_size=vocab_size, embed_dim=word_dim)
...
class OarTorso(hk.Module):
    ...
    def unbatched(self, inputs: observation_action_reward.OAR):
        ...
        # get action one-hot
        action = jax.nn.one_hot(inputs.action, num_classes=self._num_actions)
        ...
        return self._output_fn(image=image, task=task, action=action, reward=reward)


class DummyRNN(hk.RNNCore):
    def __call__(
        self, inputs: jax.Array, prev_state: hk.LSTMState
