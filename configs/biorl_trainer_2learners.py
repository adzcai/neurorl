...
from acme.jax import experiments, types, utils, variable_utils
...
def make_online_replay_tables(
    ...
    replay_tables = [
        reverb.Table.queue(
            name=replay_table_name, max_size=batch_size, signature=signature
        )
    ]
    ...
    return disable_insert_blocking(replay_tables)
    ...
    return [
        reverb.Table(
            ...
            signature=sw.infer_signature(
                configs=_make_adder_config(step_spec, sequence_length, sequence_period),
                step_spec=step_spec,
            ),
        )
    ]
...
def run_experiment(
    ...
    environment_spec = environment_spec or specs.make_environment_spec(environment)

    # Create the networks and policy.
    # policy is uses to select actions
    networks = network_factory(environment_spec)
    policy = get_actor_core(
        networks=networks, environment_spec=environment_spec, evaluation=False
    )
    ...
    # --------------------------
    iterator_batch_size, ragged = divmod(config.online_batch_size, jax.device_count())
    if ragged:
        raise ValueError(
    ...
    online_dataset = utils.multi_device_put(
    ...
    offline_adder = structured.StructuredAdder(
        ...
        configs=_make_adder_config(
            step_spec, offline_sequence_length, offline_sequence_period
        ),
        step_spec=step_spec,
    )
  ...
  """
    ...
    while True:
        ...
        episode_results.update(
            {"episode_length": episode_steps, "episode_return": episode_return}
        )
        for observer in observers:
...
def make_environment(
    seed: int, level="BabyAI-GoToRedBallNoDists-v0", evaluation: bool = False, **kwargs
) -> dm_env.Environment:
