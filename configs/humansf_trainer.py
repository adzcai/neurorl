...
flags.DEFINE_string(
    "parallel",
    "none",
    "none: run 1 experiment. sbatch: run many experiments with SBATCH. ray: run many experiments with say. use sbatch with SLUM or ray otherwise.",
)
flags.DEFINE_bool("debug", False, "If in debugging mode, only 1st config is run.")
flags.DEFINE_bool(
    "make_path", True, "Create a path under `FLAGS>folder` for the experiment"
)
flags.DEFINE_bool("auto_name_wandb", True, "automatically name wandb.")
FLAGS = flags.FLAGS
...
def make_keyroom_object_test_env(
    seed: int,
    setting: TestOptions,
    room_size: int = 6,
    ...
    **kwargs,
    ...
    if setting == TestOptions.shape.value:
        ...
        for c in ["blue", "yellow"]:
            train_tasks.append(
                ObjectTestTask(source="shape", init="ball", floor=c, w="box")
            )
            test_tasks.append(
                ObjectTestTask(source="shape", init="ball", floor=c, w="ball")
            )

            train_tasks.append(
                ObjectTestTask(source="shape", init="box", floor=c, w="ball")
            )
            test_tasks.append(
                ObjectTestTask(source="shape", init="box", floor=c, w="box")
            )
        ...
        train_tasks = [
            ObjectTestTask(
                floor="red", init="ball", w="box", floor2task_color=floor2task_color
            ),
            ObjectTestTask(
                floor="green", init="box", w="ball", floor2task_color=floor2task_color
            ),
        ]

        test_tasks = [
            ObjectTestTask(
                floor="red", init="ball", w="ball", floor2task_color=floor2task_color
            ),
            ObjectTestTask(
                floor="red", init="box", w="ball", floor2task_color=floor2task_color
            ),
            ObjectTestTask(
                floor="green", init="box", w="box", floor2task_color=floor2task_color
            ),
            ObjectTestTask(
                floor="green", init="ball", w="box", floor2task_color=floor2task_color
            ),
        ]
    ...
    gym_wrappers.append(
        functools.partial(minigrid.wrappers.RGBImgPartialObsWrapper, tile_size=8)
    )
...
def setup_experiment_inputs(
    ...
    if agent == "flat_q":
        ...
        from td_agents import basics, q_learning
        ...
        env_kwargs["object_options"] = (
            False  # has no mechanism to select from object options since dependent on what agent sees
        )

    elif agent == "flat_usfa":
        from td_agents import basics, usfa
        ...
        # NOTE: main differences below
        network_factory = functools.partial(usfa.make_minigrid_networks, config=config)
        env_kwargs["object_options"] = (
            False  # has no mechanism to select from object options since dependent on what agent sees
        )
        ...
        env_kwargs["object_options"] = (
            True  # has no mechanism to select from object options since dependent on what agent sees
        )
    else:
    ...
    environment_factory = functools.partial(make_environment_fn, **env_kwargs)
