...
// launch parallel job
python configs/remove_trainer.py \
  --search='initial' \
  --parallel='sbatch' \
  --num_actors=1 \
  --use_wandb=True \
...
from envs.blocksworld import remove
...
def observation_encoder(
  ...
  In the case of parsing blocks, 
    it creates embeddings for remove state, 
...
class QObserver(basics.ActorObserver):
  ...
  def get_metrics(self, max_steps:int = configurations['remove']['max_steps']) -> Dict[str, any]:
    ...
    print('\n\nlogging!')
    import envs.blocksworld.cfg as bwcfg
    max_steps = bwcfg.configurations['remove']['max_steps']
    curriculum = bwcfg.configurations['remove']['cur_curriculum_level']
    global up_pressure, down_pressure, UP_PRESSURE_THRESHOLD, DOWN_PRESSURE_THRESHOLD, UP_REWARD_THRESHOLD, DOWN_REWARD_THRESHOLD
    tmp_down_threshold = DOWN_PRESSURE_THRESHOLD * (curriculum+1) if 0<=curriculum<=bwcfg.configurations['remove']['maxnprocesses'] else DOWN_PRESSURE_THRESHOLD*bwcfg.configurations['remove']['maxnprocesses'] # adjust threshold for higher curriculum
    ...
    if up_pressure >= UP_PRESSURE_THRESHOLD: # up pressure reached threshold
      if 0<=curriculum<=bwcfg.configurations['remove']['maxnprocesses']-1:
        curriculum += 1
        print(f'up_pressure reached threshold, increasing curriculum from {curriculum-1} to {curriculum}')
      elif curriculum==bwcfg.configurations['remove']['maxnprocesses']:
        curriculum = -1
        print(f"up_pressure reached threshold, increasing curriculum from {bwcfg.configurations['remove']['maxnprocesses']} to {curriculum}")
      elif curriculum==-1: 
      ...
      if 1<=curriculum<=bwcfg.configurations['remove']['maxnprocesses']:
        curriculum -= 1
        print(f'down_pressure reached threshold, decreasing curriculum from {curriculum+1} to {curriculum}')
      elif curriculum==-1:
        curriculum = bwcfg.configurations['remove']['maxnprocesses']
    ...
    bwcfg.configurations['remove']['cur_curriculum_level'] = curriculum # update curriculum in cfg file
    print('logging ends\n\n')
    return results
...
def sweep(search: str = 'default'):
  if search == 'initial':
    space = [
        {
            "group": tune.grid_search(['.95remlong']),
