...
from td_agents import basics, muzero, q_learning
...
def observation_encoder(
    inputs: acme_wrappers.observation_action_reward.OAR, num_actions: int
):
    ...
    def fn(x):
        x = jnp.concatenate(
            (
                ...
                jax.nn.one_hot(x.action, num_actions),
            )
        )  # [A]
...
def make_qlearning_networks(
    ...
    def make_core_module() -> q_learning.R2D2Arch:
        ...
        observation_fn = functools.partial(observation_encoder, num_actions=num_actions)
        return q_learning.R2D2Arch(
            torso=hk.to_module(observation_fn)("obs_fn"),
            memory=networks.DummyRNN(),  # nothing happens
            head=duelling.DuellingMLP(num_actions, hidden_sizes=[config.q_dim]),
        )
...
def make_muzero_networks(
    env_spec: specs.EnvironmentSpec, config: muzero.Config, **kwargs
) -> muzero.MuZeroNetworks:
    ...
    def make_core_module() -> muzero.MuZeroNetworks:
        ...
        def transition_fn(action: int, state: State):
            ...
            def _transition_fn(action_onehot, state):
                ...
                # action: [A]
                # state: [D]
                out = muzero_mlps.SimpleTransition(num_blocks=config.transition_blocks)(
                    action_onehot, state
                )
        ...
        ###########################
        root_value_fn = hk.nets.MLP((128, 32, config.num_bins), name="pred_root_value")
        root_policy_fn = hk.nets.MLP((128, 32, num_actions), name="pred_root_policy")
...
def make_environment(
    seed: int, difficulty: int = 1, evaluation: bool = False, **kwargs
) -> dm_env.Environment:
    """Loads environments."""
    del seed
    del evaluation

    _, input_stacks, goal_stacks = mental_blocks.create_random_problem(
        difficulty=difficulty
    )

    # create simulator
    environment = mental_blocks.Simulator(
        input_stacks=input_stacks, goal_stacks=goal_stacks
    )
...
def setup_experiment_inputs(
    ...
    # -----------------------
    environment_factory = functools.partial(make_environment, **env_kwargs)
    ...
    # -----------------------
    observers = [
        utils.LevelAvgReturnObserver(reset=50, get_task_name=lambda e: "task"),
    ]
...
def train_single(
    ...
    experiment_config_inputs = setup_experiment_inputs(
        agent_config_kwargs=agent_config_kwargs, env_kwargs=env_kwargs, debug=debug
    )
