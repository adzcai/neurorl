...
BASE_BLOCK_REWARD = (
    3  # base reward for getting 1 block correct, cumulates with more blocks correct
)
...
class Simulator:
    def __init__(
        ...
        reward_decay_factor=BLOCK_REWARD_DECAY_FACTOR,
        ...
        self.state_size = (
            self.state.size
        )  # total number of elements in flattened state matrix
        ...
        self.max_episode_reward = sum(
            [
                self.base_block_reward
                / (self.reward_decay_factor ** (intersection + 1))
                for istack in range(len(goal_stacks))
                for jblock in range(len(goal_stacks[istack]))
                for intersection in range(jblock + 1)
            ]
            + [
                -5 * action_cost
                for istack in range(len(goal_stacks))
                for _ in range(len(goal_stacks[istack]))
            ]
            + [-1 * action_cost for _ in range(len(goal_stacks))]
            + [-action_cost, -action_cost]
        )
        ...
        self.goal = [
            [-1 for _ in range(max_blocks)] for _ in range(max_stacks)
        ]  # [max_stacks, max_blocks]
        for istack in range(
            len(goal_stacks)
        ):  # a stack [-1, -1, ..., top block, ..., bottom block]
    ...
    def __parse_input(self):
        for istack in range(self.max_stacks):
            for jblock in range(self.max_blocks):
                self.state[istack * self.max_blocks + jblock, :] = (
                    0  # initialize one-hot encoding
                )
                if self.input_stacks[istack][jblock] != -1:  # encode nonempty block
                    self.state[
                        istack * self.max_blocks + jblock,
                        self.input_stacks[istack][jblock],
                    ] = 1
        self.state[
            self.max_blocks
            * (self.max_stacks * 2 + 1) : self.max_blocks
            * (self.max_stacks * 2 + 1)
            + self.max_stacks,
            :,
        ] = 0  # clear intersection record
        self.state[
            self.max_blocks * self.max_stacks : self.max_blocks * self.max_stacks
            + self.max_blocks,
            :,
        ] = 0  # clear table stacks
    ...
    def __parse_goal(self):
        for istack in range(self.max_stacks):
            for jblock in range(self.max_blocks):
                self.state[
                    (istack + self.max_stacks + 1) * self.max_blocks + jblock, :
                ] = 0  # initialize one-hot encoding
                if self.goal[istack][jblock] != -1:  # encode nonempty block
                    self.state[
                        (istack + self.max_stacks + 1) * self.max_blocks + jblock,
                        self.goal[istack][jblock],
                    ] = 1
    ...
    def step(self, action_idx):
        ...
        input_parsed = np.any(
            self.state[-2] == 1
        )  # True if any 1 exists (whole vector will be 1s if parsed already)
        goal_parsed = np.any(
            self.state[-1] == 1
        )  # True if any 1 exists (whole vector will be 1s if parsed already)
        ...
        if (not input_parsed) or (not goal_parsed):
            if (
                (not input_parsed)
                and (not goal_parsed)
                and (action_name != "parse_input")
                and (action_name != "parse_goal")
            ):
            ...
            if np.all(
                self.state[cur_pointer * self.max_blocks + self.max_blocks - 1] == 0
            ):  # nothing to remove, cur stack is empty
                reward -= self.action_cost
                ...
                block_id = -1
                for i in range(
                    cur_pointer * self.max_blocks,
                    cur_pointer * self.max_blocks + self.max_blocks,
                ):  # from top block to bottom
                    if np.any(
                        self.state[i] == 1
                    ):  # find the first nonempty block in stack
                        pop_idx = i
                ...
                for i in range(
                    ...
                    self.max_blocks * self.max_stacks + self.max_blocks,
                ):
                    if np.all(self.state[i] == 0):  # find the first empty table stack
                        self.state[i, block_id] = (
                            1  # put the removed block to the tabel
                        )
                        break
            ...
            if np.all(
                self.state[self.max_blocks * self.max_stacks + table_pointer] == 0
            ):  # nothing to add, table stack is empty
                reward -= self.action_cost
                print("\tnothing to add, table stack empty") if self.verbose else 0
            elif np.any(
                self.state[cur_pointer * self.max_blocks] == 1
            ):  # intent to add to cur stack, but stack full
                (
                    print(
                        "\tintend to add to full stack, last block in stack is",
                        self.state[cur_pointer + self.max_blocks - 1],
                    )
                    if self.verbose
                    else 0
                ...
                new_block = np.argmax(
                    self.state[self.max_blocks * self.max_stacks + table_pointer]
                )  # the new blockid to be added
                self.state[
                    self.max_blocks * self.max_stacks + table_pointer, new_block
                ] = 0  # remove the block from table
                for i in range(
                    cur_pointer * self.max_blocks + self.max_blocks - 1,
                    cur_pointer * self.max_blocks - 1,
                    -1,
                ):  # from bottom to top
                    if np.all(self.state[i] == 0):  # the first empty block on top
                        self.state[i, new_block] = (
                            1  # add the block to the top of cur stack
                        )
                        break
                r, terminated = self.__readout_reward()
                reward += r
        ...
        for istack in range(self.max_stacks):
            ...
            for jblock in range(self.max_blocks):
                if np.any(self.state[istack * self.max_blocks + jblock] == 1):
                    stack.append(
                        np.argmax(self.state[istack * self.max_blocks + jblock]).item()
                    )
                else:
                    stack.append(-1)
    ...
    def __readout_reward(self):
        ...
        intersection_record = self.state[
            self.max_blocks
            * (self.max_stacks * 2 + 1) : self.max_blocks
            * (self.max_stacks * 2 + 1)
            + self.max_stacks,
        ...
        intersections = [
            0
        ] * self.max_stacks  # temporary intersection for the current readout
        for istack in range(self.max_stacks):  # iterate each stack
            intersections[istack], _ = self.__intersection(
                cur_stacks[istack], self.goal[istack]
            )  # number of blocks that match
            (
                print(
                    "intersections",
                    intersections,
            ...
            if intersections[istack] != 0 and (
                intersection_record[istack][intersections[istack] - 1] == 0
            ):  # new match
                score += sum(
                    [
                        base_block_reward / (reward_decay_factor ** (d + 1))
                        for d in range(intersections[istack])
                    ]
                )
                ...
                score += 0
        all_correct = (
            sum(intersections) == self.num_valid_blocks
    ...
    def __create_state_representation(self):
        ...
        self.state[-4, 0] = (
            1  # initialize the cur pointer to be the first stack (one-hot encoding)
        )
        self.state[-3, 0] = (
            1  # initialize the table pointer to be the first table stack (one-hot encoding)
        )
        return self.state
    ...
    def expert_demo(self):
        ...
        final_actions.append(
            list(self.action_dict.keys())[list(self.action_dict.values()).index(action)]
        )
        action = "parse_goal"
        final_actions.append(
            list(self.action_dict.keys())[list(self.action_dict.values()).index(action)]
        ...
        for istack in range(self.max_stacks):
            for jblock in range(self.max_blocks):  # iterating from top to bottom
                if (
                    self.input_stacks[istack][jblock] == -1
                ):  # have not reached valid block yet
                    continue
                action = "remove"
                final_actions.append(
                    list(self.action_dict.keys())[
                        list(self.action_dict.values()).index(action)
                    ]
                )
                table.append(
                    self.input_stacks[istack][jblock]
                )  # record the blocks put on table
            if (
                self.max_stacks > 1 and istack != self.max_stacks - 1
            ):  # move to next stack if applicable
                action = "next_stack"
                final_actions.append(
                    list(self.action_dict.keys())[
                        list(self.action_dict.values()).index(action)
        ...
        for istack in range(self.max_stacks):
            for jblock in range(
                self.max_blocks - 1, -1, -1
            ):  # iterating from bottom to top in each stack
                ...
                (
                    print("blocktoadd:{}, table loc:{}".format(blocktoadd, loc))
                    if self.verbose
                    else 0
                )
                if (
                    loc - table_pointer > 0
                ):  # should move to the right to reach the table stack
                ...
                table_pointer = (
                    loc  # table pointer is now moved to the table stack to be added
                )
                if (
                    istack - cur_pointer > 0
                ):  # cur stack pointer need to move to the right
                    ...
                    istack - cur_pointer < 0
                ):  # cur stack pointer need to move to the left
    ...
    def test(self, verbose=False, use_expert=False):
        ...
        if not use_expert:
            for t in range(self.max_steps + 10):
                ...
                total_reward += reward
                print(
                    "t={}, reward={}, action_idx={}, action={}, done={}, next state\n{}".format(
                        t,
                        reward,
                        action_idx,
                        self.action_dict[action_idx],
                        done,
                        self.state,
                    )
                )
...
def _convert_to_spec(space: Any, name: Optional[str] = None) -> types.NestedSpec:
    """
    ...
    if isinstance(space, int):  # scalar int for number of actions
        ...
        try:
            ...
            raise ValueError(
                "Converting integer to dm_env spec, but name is not action"
            )
        return specs.BoundedArray(
            shape=(), dtype=dtype, minimum=min_val, maximum=max_val, name=name
        )
        ...
        try:
            ...
            raise ValueError(
                "Converting np.ndarray to dm_env spec, but name is not 'observation'"
            )
        return specs.BoundedArray(
            shape=space.shape,
            dtype=space.dtype,
        ...
        return {key: _convert_to_spec(value, key) for key, value in space.items()}
    else:
        raise ValueError(
            "Unsupported data type for conversion to dm_env spec: {}".format(space)
        )


def create_random_problem(
    max_num_blocks=MAX_BLOCKS, max_num_stacks=MAX_STACKS, difficulty=None
):
    while True:
        num_blocks = (
            random.choice(list(range(2, max_num_blocks + 1)))
            if difficulty == None
            else difficulty
        )
        # input stacks
        ...
        for _ in range(max_num_stacks):
            if len(available_blocks) == 0:
                continue
            num_blocks_istack = random.choice(
                list(range(len(available_blocks)))
            )  # if max_num_stacks>1 else len(available_blocks)
            curstack = []
