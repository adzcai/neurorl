...
from minigrid.core.world_object import Ball, Box, Floor, Key
...
class ObjectTestTask:
    ...
    def task_vector(self):
        vector = self.task_features_cnstr.task(
            obj2reward={
                (self.goal_color(), "key"): 0.1,
                (self.goal_color(), "task_room"): 0.5,
    ...
    def step(self, env):
        ...
        if (
            ...
            and env.carrying.type == type
            and env.carrying.color == color
        ):
            terminal = True
        ...
        if not self.entered_task_room and self.agent_in_task_room(env):
        ...
        if (
            not self.picked_up_key
            and env.carrying is not None
...
class KeyRoomObjectTest(LevelGen):
    ...
    def gen_mission(self):
        ...
        for room_idx, color in enumerate(colors):
            ...
            _, _ = self.add_door(
                *center_room, door_idx=room_idx, color=color, locked=self.rooms_locked
            )
            ...
            for key, counts in self.results.items():
                ...
                for i, obj in enumerate(objs):
                    ...
                    shape = shape_mapping.get(
                        shape_name, ""
                    )  # Empty string if shape not found
                    ax.text(
                        i, -0.1, shape, rotation=0, ha="center", va="top", color=color
                    )
