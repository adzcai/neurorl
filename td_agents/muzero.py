...
from library import muzero_mlps, utils
...
StateFn = Callable[[Params, PRNGKey, Observation, State], Tuple[NetworkOutput, State]]
...
class MuZeroNetworks:
    ...
    apply: StateFn
    unroll: StateFn
    init_recurrent_state: Callable[[PRNGKey, Optional[BatchSize]], State]
    apply_model: ModelFn
...
class Config(basics.Config):
    ...
    scalar_step_size: Optional[float] = 0.25  # step size between bins
    max_scalar_value: float = (
        10.0  # number of bins for two-hot rep  max_scalar_value: float = 10.0  # number of bins for two-hot rep
    )
    v_target_source: str = (
        "reanalyze"  # this interpolates between mcts output vs. observed return
    )
...
def muzero_optimizer_constr(config, initial_params=None):
    ...
    if config.weight_decay > 0.0:
        ...
        def decay_fn(module_name, name, value):
            return True if name == "w" else False
...
class MuZeroLossFn(basics.RecurrentLossFn):
    ...
    discretizer: utils.Discretizer = utils.Discretizer(max_value=10, num_bins=101)
    ...
    simulation_steps: float = (
        0.5  # how many time-steps of simulation to learn model with
    )
    ...
    state_from_preds: Callable[[RootOutput], jax.Array] = lambda preds: preds.state
    ...
    def error(
        ...
        **kwargs,
        ...
        loss_fn = jax.vmap(loss_fn, in_axes=(1, 1, None, 1, None), out_axes=0)
    ...
    def compute_target(
        ...
        **kwargs,
        ...
        target_values = self.discretizer.logits_to_scalar(target_preds.value_logits)
        ...
        invalid_actions = self.get_invalid_actions(batch_size=target_values.shape[0])
        ...
        # Values
        # ---------------
        discounts = (data.discount[:-1] * self.discount).astype(target_values.dtype)
        ...
        if self.value_target_source == "mcts":
            ...
            reanalyze = distrax.Bernoulli(probs=self.reanalyze_ratio).sample(
                seed=rng_key
            )
    ...
    def learn(
        ...
        **kwargs,
        ...
        # [T]
        root_policy_ce = policy_loss_fn(policy_probs_target, online_preds.policy_logits)
        # []
        ...
        uniform_policy = jnp.ones((self.simulation_steps, num_actions)) / num_actions
        invalid_actions = self.get_invalid_actions(batch_size=self.simulation_steps)
        if invalid_actions is not None:
            valid_actions = 1 - invalid_actions
            uniform_policy = valid_actions / valid_actions.sum(-1, keepdims=True)

        policy_model_target = jnp.concatenate((policy_probs_target, uniform_policy))
        ...
        vmap_roll = jax.vmap(
            functools.partial(rolling_window, size=self.simulation_steps), 1, 2
        )
        ...
        simulation_actions = rolling_window(simulation_actions, self.simulation_steps)
        ...
        def model_unroll(key, state, actions):
            ...
            model_output, _ = networks.unroll_model(params, model_key, state, actions)
            return model_output
        ...
        model_outputs = model_unroll(
            model_keys,
            self.state_from_preds(online_preds),
            simulation_actions,
        )
        ...
        model_policy_loss = self.model_policy_coef * raw_model_policy_loss
        raw_model_value_loss = utils.episode_mean(model_value_loss, value_mask[:nsteps])
        model_value_loss = self.model_value_coef * raw_model_value_loss
        raw_model_reward_loss = utils.episode_mean(
            model_reward_loss, reward_mask[:nsteps]
        )
        reward_loss = self.model_reward_coef * raw_model_reward_loss
    ...
    def get_invalid_actions(self, batch_size):
        ...
        if self.invalid_actions.ndim < 2:
            self.invalid_actions = jax.numpy.tile(self.invalid_actions, (batch_size, 1))
...
def make_network(
    ...
    def init_recurrent_state(
        key: jax_types.PRNGKey, batch_size: Optional[int] = None
    ) -> State:
        no_params = None
...
def mcts_select_action(
    mcts_policy: Optional[Union[mctx.muzero_policy, mctx.gumbel_muzero_policy]],
    discretizer: utils.Discretizer,
    params: networks_lib.Params,
    observation: networks_lib.Observation,
    state: basics.ActorState[actor_core_lib.RecurrentState],
...
def get_actor_core(
    ...
    if config.action_source == "policy":
        select_action = functools.partial(
            policy_select_action, networks=networks, evaluation=evaluation
        )
    ...
    def init(
        ...
        initial_core_state = networks.init_recurrent_state(state_rng)
...
class MuZeroArch(hk.RNNCore):
    ...
    def initial_state(self, batch_size: Optional[int] = None, **unused_kwargs) -> State:
        return self._state_fn.initial_state(batch_size)

    def __call__(
        self, inputs: jax.Array, state: State  # [...]  # [...]
    ...
    def unroll(
        self, inputs: jax.Array, state: State  # [T, B, ...]  # [B, ...]
    ) -> Tuple[RootOutput, State]:
        ...
        all_hidden, new_state = hk.static_unroll(self._state_fn, state_input, state)
    ...
    **kwargs,
    ...
    def make_core_module() -> MuZeroNetworks:
        ...
        def transition_fn(action: int, state: State):
            action_onehot = jax.nn.one_hot(action, num_classes=num_actions)
            ...
            def _transition_fn(action_onehot, state):
                ...
                out = muzero_mlps.Transition(
                    channels=config.state_dim, num_blocks=config.transition_blocks
                )(action_onehot, state)
    ...
    return make_network(
        environment_spec=env_spec, make_core_module=make_core_module, **kwargs
    )
