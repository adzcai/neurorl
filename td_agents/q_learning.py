...
from acme.jax.networks import base, duelling
...
class R2D2LossFn(basics.RecurrentLossFn):
    ...
    def error(
        ...
        discounts = (data.discount * self.discount).astype(
            self.extract_q(online_preds).dtype
        )
        rewards = data.reward
...
class R2D2Arch(hk.RNNCore):
    ...
    def unroll(
        ...
        core_outputs, new_states = hk.static_unroll(self._memory, embeddings, state)
