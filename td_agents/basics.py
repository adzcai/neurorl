...
# initializations
ValueInitFn = Callable[
    [networks_lib.PRNGKey, Observation, hk.LSTMState], networks_lib.Params
]

# calling networks
RecurrentStateFn = Callable[[networks_lib.Params], hk.LSTMState]
ValueFn = Callable[[networks_lib.Params, Observation, hk.LSTMState], networks_lib.Value]
...
def param_sizes(params):
    ...
    sizes = group_named_values(dictionary=tree.map_structure(jnp.size, params))
...
class Config(r2d2.R2D2Config):
    ...
    # ----------------
    linear_epsilon: bool = (
        False  # whether to use linear or sample from log space for all actors
    )
    epsilon_begin: float = 0.9
...
class RecurrentLossFn(learning_lib.LossFn):
    ...
    def __call__(
        ...
        if self.clip_rewards:
            data = data._replace(
                reward=jnp.clip(data.reward, -self.max_abs_reward, self.max_abs_reward)
            )
        ...
        if burn_in_length:
            ...
            key_grad, key1, key2 = jax.random.split(key_grad, 3)
            _, target_state = unroll(target_params, key2, burn_obs, target_state)
        ...
        online_preds, online_state = unroll(
            params, key1, data.observation, online_state
        )
        key_grad, key1, key2 = jax.random.split(key_grad, 3)
        ...
        # [B]
        importance_weights = (1.0 / (probs + 1e-6)).astype(batch_loss.dtype)
        ...
        total_aux_elem_error = jnp.zeros(
            elemwise_error.shape, dtype=elemwise_error.dtype
        )
        ...
        mean_priority = (1 - self.max_priority_weight) * jnp.mean(
            total_elemwise_error, axis=0
        )
...
class SGDLearner(learning_lib.SGDLearner):
    ...
    def __init__(
        ...
        def sgd_step(
            state: TrainingState, batch: reverb.ReplaySample
        ) -> Tuple[TrainingState, LossExtra]:
            ...
            if isinstance(target_update_period, float):
                assert (
                    target_update_period > 0.0 and target_update_period < 1.0
                ), "incorrect float"
                target_params = optax.incremental_update(
                    new_params, state.target_params, target_update_period
                )
        ...
        def update_priorities(reverb_priorities: ReverbUpdate) -> None:
            ...
            replay_client.mutate_priorities(
                table=replay_table_name, updates=dict(zip(keys, priorities))
            )
...
class BasicActor(core.Actor, Generic[actor_core_lib.State, actor_core_lib.Extras]):
    ...
    def observe(self, action: network_lib.Action, next_timestep: dm_env.TimeStep):
        if self._adders:
            for adder in self._adders:
                adder.add(action, next_timestep, extras=self._get_extras(self._state))
...
class Builder(r2d2.R2D2Builder):
    ...
    def make_learner(
        ...
        return SGDLearner(
            ...
            optimizer_cnstr=functools.partial(
                self.optimizer_cnstr, config=self._config
            ),
    ...
    def make_policy(
        ...
        return self._get_actor_core_fn(
            networks=networks, evaluation=evaluation, config=self._config
        )
    ...
    def make_actor(
        ...
        return self.ActorCls(policy, random_key, variable_client, adder, backend="cpu")
...
def get_actor_core(
    ...
    def init(rng: networks_lib.PRNGKey) -> ActorState[actor_core_lib.RecurrentState]:
