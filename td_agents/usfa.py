...
from library.utils import episode_mean, expand_tile_dim, make_episode_mask
...
class UsfaLossFn(basics.RecurrentLossFn):
    ...
    index_cumulants: Callable[[jnp.ndarray], jnp.ndarray] = lambda x: x[:-1]

    def error(
        self, data, online_preds, online_state, target_preds, target_state, **kwargs
    ):
        # ======================================================
        # Prepare Data
        # ======================================================
        ...
        td_error_fn = functools.partial(
            rlax.transformed_n_step_q_learning, n=self.bootstrap_n
        )
        ...
        td_error_fn = jax.vmap(
            td_error_fn, in_axes=(4, None, 4, None, 2, None), out_axes=3
        )
        ...
        if self.mask_loss:
            ...
            batch_loss = episode_mean(
                x=(0.5 * jnp.square(batch_td_error)), mask=episode_mask[:-1]
            )
        else:
...
class SfGpiHead(hk.Module):
    ...
    def __call__(
        ...
        if self.nsamples > 0:
            ...
            policies = jnp.concatenate(
                (policy_base, policy_samples), axis=-2
            )  # [N+1, D_w]
        else:
            policies = jnp.expand_dims(policy, axis=-2)  # [1, D_w]

        return self.sfgpi(usfa_input=usfa_input, policies=policies, task=policy)
    ...
    def evaluate(
        ...
        preds = self.sfgpi(usfa_input=usfa_input, policies=policies, task=task)
    ...
    def sfgpi(
        ...
        def compute_sf_q(
            sf_input: jnp.ndarray, policy: jnp.ndarray, task: jnp.ndarray
        ) -> jnp.ndarray:
            ...
            # dot-product: A
            q_values = jax.vmap(dot, in_axes=(0, None), out_axes=0)(sf, task)
...
class UsfaArch(hk.RNNCore):
    ...
    def unroll(
        ...
        core_outputs, new_states = hk.static_unroll(self._memory, memory_input, state)
...
def make_minigrid_networks(
    env_spec: specs.EnvironmentSpec, config: Config
) -> networks_lib.UnrollableNetwork:
    ...
    def make_core_module() -> UsfaArch:
        vision_torso = networks.AtariVisionTorso(out_dim=config.state_dim)
        ...
        return UsfaArch(
            torso=observation_fn, memory=hk.LSTM(config.state_dim), head=usfa_head
        )
...
class ObjectOrientedUsfaArch(hk.RNNCore):
    ...
    def make_object_oriented_usfa_inputs(
        ...
        action_embdder = lambda x: hk.Linear(128, w_init=hk.initializers.RandomNormal)(
            x
        )
        object_embdder = lambda x: hk.Linear(128, w_init=hk.initializers.RandomNormal)(
            x
        )
